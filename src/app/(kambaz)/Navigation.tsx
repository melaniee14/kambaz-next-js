    ...
    <ListGroup id="wd-kambaz-navigation" style={{width: 120}}
      ...
      <ListGroupItem as={Link} href="/account"
        className={`text-center border-0 bg-black
            ${pathname.includes("Account".toLowerCase()) ? "bg-white text-danger" : "bg-black text-white"}`}>
        <FaRegCircleUser
          className={`fs-1 ${pathname.includes("Account".toLowerCase()) ? "text-danger" : "text-white"}`} />
        <br />
        Account
      </ListGroupItem>
      {links.map((link) => (
